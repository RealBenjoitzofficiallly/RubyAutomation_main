    ...
    while True:
        ...
        if response:
            ...
            if scriptVersion == final:
                ...
                with open('RubyConfig.json', 'r') as f:
                    RubyConfig = json.load(f)
...
def only_owner(): 
    async def predicate(ctx):
        with open('RubyConfig.json', 'r') as f:
            RubyConfig = json.load(f)
        only_ownerid = [int(x) for x in RubyConfig['MISC']['DISCORD']['ONLY_OWNERID']]
        return ctx.author.id in only_ownerid
    return commands.check(predicate)
...
@bot.command()
@only_owner()
async def addisowner(ctx, user_id: int):
...
@bot.command()
@only_owner()
async def addonlyowner(ctx, user_id: int):
    with open('RubyConfig.json', 'r') as file:
        RubyConfig = json.load(file)
    
    only_ownerid = RubyConfig["MISC"]["DISCORD"]["ONLY_OWNERID"]
    
    if str(user_id) not in only_ownerid:
        only_ownerid.append(str(user_id))
        RubyConfig["MISC"]["DISCORD"]["ONLY_OWNERID"] = only_ownerid
        
        with open('RubyConfig.json', 'w') as file:
            json.dump(RubyConfig, file, indent=4)
...
@bot.command()
@only_owner()
async def removeisowner(ctx, user_id: int):
    with open('settings.json', 'r') as file:
        settings = json.load(file)
    authorized_ids = settings["MISC"]["DISCORD"]["AUTHORIZED_IDS"]
...
@bot.command()
@only_owner()
async def removeonlyowner(ctx, user_id: int):
    with open('RubyConfig.json', 'r') as file:
        RubyConfig = json.load(file)
    only_ownerid = RubyConfig["MISC"]["DISCORD"]["ONLY_OWNERID"]
    if str(user_id) in only_ownerid:
        only_ownerid.remove(str(user_id))
        RubyConfig["MISC"]["DISCORD"]["ONLY_OWNERID"] = only_ownerid
        with open('RubyConfig.json', 'w') as file:
            json.dump(RubyConfig, file, indent=4)
        embed = discord.Embed(
...
@bot.command()
@only_owner()
async def ars(ctx, minutes: Union[int, str] = None):
